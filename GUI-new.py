...
class MyFrame(Frame):
    def __init__(self):
        ...
        self.button3 = wx.Button(panel, -1, '沐浴露规格名称判别', pos=(100, 120))
        ...
        self.button6 = wx.Button(panel, -1, '将当期数据放入库中', pos=(100, 320))
        self.Bind(wx.EVT_BUTTON, self.main6, self.button6)
        self.button6.SetDefault()

        self.button7 = wx.Button(panel, -1, '将当期图片放入库中', pos=(100, 370))
        self.Bind(wx.EVT_BUTTON, self.main7, self.button7)
        self.button7.SetDefault()

        self.button8 = wx.Button(panel, -1, '牙膏名称判别', pos=(300, 120))
        self.Bind(wx.EVT_BUTTON, self.main3, self.button8)
        self.button8.SetDefault()

        self.button9 = wx.Button(panel, -1, '限量版判别', pos=(100, 270))
        self.Bind(wx.EVT_BUTTON, self.main9, self.button9)
        self.button9.SetDefault()

        self.button10 = wx.Button(panel, -1, '天猫', pos=(100, 420))
        self.Bind(wx.EVT_BUTTON, self.tianmao, self.button10)
        self.button10.SetDefault()

        self.button11 = wx.Button(panel, -1, '京东', pos=(300, 420))
        self.Bind(wx.EVT_BUTTON, self.jingdong, self.button11)
        self.button11.SetDefault()

        self.button12 = wx.Button(panel, -1, '全部', pos=(500, 420))
        self.Bind(wx.EVT_BUTTON, self.total, self.button12)
        self.button12.SetDefault()
        # self.filepath1 = r'D:\工作交接1\03上新监测项目实施/01沐浴露监测/01测试数据/01旗舰店数据/2018-7-31旗舰店数据/'   #修改日期
        # self.filepath2 = r'D:\工作交接1\03上新监测项目实施/01沐浴露监测/02成品数据/旗舰店数据/2018-6-29/'   #修改日期
        # self.filepath3 = r'D:\工作交接1\03上新监测项目实施/01沐浴露监测/02成品数据/旗舰店数据/2018-7-31/'  #修改日期
        # self.filepath4 = r'D:\工作交接1\03上新监测项目实施\01沐浴露监测\02成品数据\旗舰店数据/'
        # self.filepath5 = r'D:\工作交接1\03上新监测项目实施\A_judge_pic\muyulu/'
        # self.date = '7.31/'  #修改日期

        self.filepath1 = r'D:\工作交接1\03上新监测项目实施\04中华项目监测/01测试数据/01旗舰店数据/2018-6-13旗舰店数据/'  #修改日期
        self.filepath2 = r'D:\工作交接1\03上新监测项目实施\04中华项目监测/02成品数据/旗舰店数据/2018-5-31/'  #修改日期
        self.filepath3 = r'D:\工作交接1\03上新监测项目实施\04中华项目监测/02成品数据/旗舰店数据/2018-6-13/'  #修改日期
        self.filepath4 = r'D:\工作交接1\03上新监测项目实施\04中华项目监测\02成品数据\旗舰店数据/'
        self.filepath5 = r'D:\工作交接1\03上新监测项目实施\A_judge_pic\toothpaste/'
        self.word = ['天猫/', '京东/']#
        self.date = '6.13/'  #修改日期
    def tianmao(self,e):
        self.word = ['天猫/']
    ...
    def main9(self,e):
        ##
        for name1 in self.word:
            self.xianliang_judge(self.filepath3+name1)
        print('限量版查找完成')
    ...
    def main3(self,e):
        ...
        for name in self.word:
            self.new_url_judge(self.filepath2 + name, self.filepath3 + name)
            self.same_url_zd_judge_mc(self.filepath3 + name)
            self.diff_url_zd_judge_mc(self.filepath3 + name)
        print('牙膏名称判别完成')
    ...
    #查找限量版产品，把所有找到的产品放在一个文件中
    def xianliang_judge(self,filepath):
        for name2 in os.listdir(filepath):
            print(name2)
            if name2 == '提取数据1':
                df_xl_all = pd.DataFrame(columns=['商品促销标题'])
                if not os.path.exists(filepath + '限量版/'):
                    os.mkdir(filepath + '限量版/')
                for name3 in os.listdir(filepath + name2 + '/'):

                    df = pd.read_excel(filepath + name2 + '/' + name3)
                    df_xl = pd.DataFrame(columns=df.columns)
                    xl = [col for col in df.columns.values if ('单品' in col) or ("商品促销标题" in col)]

                    for j in range(len(df)):
                        for i in xl:
                            if '限量' in str(df.loc[j, i]):
                                df_xl = df_xl.append(df.loc[j], ignore_index=True)
                                break
                    df_xl_all = pd.concat([df_xl, df_xl_all], ignore_index=True)
                df_xl_all.to_excel(filepath + '限量版/限量版.xlsx', index=False)


    #文件夹状态下：新链接名称判别
    # filePath表示新链接所在的文件夹路径
    def diff_url_zd_judge_mc(self,filePath):
        filename = self.get_filename(filePath + '不同链接数据提取/')
        # print(filename)
        for name in filename:
            df = self.diff_zd_judge_mc(filePath, name)
    ...
    # 文件夹状态下：旧链接名称判别
    # filePath表示旧链接所在的文件夹路径
    def same_url_zd_judge_mc(self,filePath):
        filename = self.get_filename(filePath + '相同链接数据提取/')
        # print(filename)
        for name in filename:
            df = self.same_zd_judge_mc(filePath, name)
    ...
    #文件状态下：旧链接名称判别
    # filePath表示旧链接所在的文件夹路径
    def same_zd_judge_mc(self,filePath, name):
    ...
    # 文件状态下：新链接名称判别
    # filePath表示新链接所在的文件夹路径
    def diff_zd_judge_mc(self,filePath, name):
        df_pool = pd.read_excel(self.filepath4 + '条件池1/' + name)
    ...
    # 新链接图片判断，将其与图片库进行对比，若存在返回图片库的图片路径
    ...
    # 文件状态下：图片相似度判断，当r、g、b三通道的像素占比均大于85%时，认为是相同的照片
                ...
                if re.search('.tmall', df2.loc[0, '页面网址']):
                    ...
                    for j in range(df1.shape[0]):
                        if url in df1.loc[j, '页面网址']:
                            pic1 = self.main(str(self.filepath5) + str(df2.loc[i, '图片链接']))
                            pic2 = self.main(str(self.filepath5) + str(df1.loc[j, '图片链接']))
    ...
    def judge_pic(self,path2, path3):
        ...
        for i in range(len(self.word)):
            for name in os.listdir(path2 + self.word[i] + '提取数据1/'):
                df = self.pic_judge(path2 + self.word[i] + '提取数据1/' + name, path3 + self.word[i] + '提取数据1/' + name)
                if os.path.isfile(path3 + self.word[i] + '图片判断/' + '相同图片/' + name):
                    os.remove(path3 + self.word[i] + '图片判断/' + '相同图片/' + name)
                df[0].to_excel(path3 + self.word[i] + '图片判断/' + '相同图片/' + name, index=False)
                if os.path.isfile(path3 + self.word[i] + '图片判断/' + '不同图片/' + name):
                    os.remove(path3 + self.word[i] + '图片判断/' + '不同图片/' + name)
                df[1].to_excel(path3 + self.word[i] + '图片判断/' + '不同图片/' + name, index=False)
    ...
    def same_zd_judge(self, filePath , name):
        df_pool = pd.read_excel(self.filepath4 + '条件池1/' + name)
        df = pd.read_excel(filePath + '相同链接数据提取/' + name)
        df['标题规格'] = df['商品促销标题'].apply(self.fun4_guige)
        df['商品id'] = np.NaN
        df['商品名称判断'] = np.NaN
        df_title_same = pd.DataFrame(columns=df.columns)
        df_title_diff = pd.DataFrame(columns=df.columns)
        ...
        for i in range(df.shape[0]):
            for j in range(df.loc[i, guige1].count()):
                if isinstance(df.loc[i, guige1[j]], str):
                    df.loc[i, guige1[j]] = df.loc[i, guige1[j]].replace('M', 'm').replace('l', 'L').replace('K', 'k').replace('G','g')
        guige2 = []
                                ...
                                if name1 == True:
                                    ...
                                    for u in range(len(guige1)):
                                        if isinstance(df.loc[i, guige1[u]], str):
                                            ...
                                            gg = df.loc[i, guige1[u]].split(' ')
                                            for k in range(len(gg)):
                                                if (gg[k].strip() not in df_pool_all) and (gg[k].strip() not in ['其他']):
                                                    name2 = True
                                                    df_title_diff.loc[i] = df.loc[i]
                                    if name2 == False:
                                        df.loc[i, '商品id'] = df_pool.loc[j, '商品id']
        ...
        if isinstance(s, str):
            ...
            for gg in s:
                if isinstance(gg, str):
                    if ('g' in gg) or ('mL' in gg) or ('L' in gg) or ('kg' in gg):
                        if re.search('[\d]', gg):
                            new.append(gg)
        for index,value in enumerate(new):
            if 'x' in new[index]:
                new[index] = new[index][:new[index].find('x')]
            elif 'X' in  new[index]:
                new[index] = new[index][:new[index].find('X')]
            else:
                continue
